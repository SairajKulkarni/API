...
const ErrorHanldler = require("../utils/errorHandler.cjs");
...
exports.loginUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(
      new ErrorHanldler("Please enter the Email & Password both", 400)
    );
  }
  ...
  if (!user) return next(new ErrorHanldler("Invalid email or password", 401));

  const isPasswordMatched = await user.comparePassword(password);

  // if user with given email id is found but the password provided is wrong
  if (!isPasswordMatched) {
    return next(new ErrorHanldler("Invalid email or password", 401));
  }
...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(new ErrorHanldler("User not found", 404));
  }
  ...
  try {
    ...
    return next(new ErrorHanldler(error.message, 500));
  }
});
...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHanldler("Reset password token is invalid or expired", 400)
    );
  }

  if (req.body.password !== req.body.confirmPassword) {
    return next(new ErrorHanldler("Passwords do not match", 400));
  }
...
exports.updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!isPasswordMatched) {
    return next(new ErrorHanldler("Old password is incorrect", 401));
  }

  if (req.body.newPassword !== req.body.confirmPassword) {
    return next(new ErrorHanldler("Passwords don't match", 400));
  }
...
exports.getAnyUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHanldler(`User does not exist with id  ${req.body.params}`, 404)
    );
  }
...
exports.deleteUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHanldler(`User does not exist with the id: ${req.params.id}`)
    );
  }
